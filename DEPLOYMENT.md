...
3. Run `powershell -File scripts/Backup-SiteFiles.ps1 -SiteRoot 'C:\\sites\\JobForFresher' -Destination 'D:\\private-backups\\JobForFresher-release.zip'`. Choose a new archive outside the website and protect access. This includes App_Data security keys/settings/resumes and uploaded logos, including hidden files. Back up environment secrets separately through the host's secret manager. The ZIP is not encrypted and is not a database backup.
4. Preview old resume migration with `powershell -File scripts/Move-LegacyResumes.ps1 -SiteRoot 'C:\\sites\\JobForFresher' -WhatIf`; then run without `-WhatIf`. The script refuses collisions, links and nested folders, and never overwrites a destination. The application only lists private files directly under App_Data/Resumes.
...
## Verify
- Admin > Launch readiness checks the current database migrations, configuration, legacy resumes and incomplete active listings.
...
- Verify private paths return no file content through the public host/CDN: /uploads/resumes/test.pdf and /App_Data/site-settings.json. IIS hidden segments and application middleware both block private data.
...
## Repeat local checks
Run `powershell -NoProfile -File tests/storage.ps1` for synthetic file migration/backup/restore checks.
